...
import numpy as np
import sys
sys.path.append("/data/jkx/Documents/correlation/CaRB")
...
def read_unified(fn):
    ...
    for X in f_data:
        ...
        for ori_args in X["ori_args"]:
            try:
                curExtraction = Extraction(pred=ori_args[0], head_pred_index=-1, sent=X["ori_sent"], confidence=-1)
            except:
                continue
        ...
        for para in X["paraphrases"]:
            ...
            for para_args in para["args"]:
                try:
                    curExtraction = Extraction(pred=para_args[0], head_pred_index=-1, sent=para["sent"], confidence=-1)
                except:
                    continue
...
def main(gold_file, pred_file, save_dir):
    ...
    for ori_sent in pred:
        ...
        for sent in pred[ori_sent]:
            ...
            auc, [p, r, f1] = bench.compare(p_extraction, Matcher.binary_linient_tuple_match)
    ...
    print("The carb scores AUC: {}, P: {}, R: {}, F1: {}".format(carb_auc, carb_p, carb_r, carb_f1))
    print("The robust scores AUC: {}, P: {}, R: {}, F1: {}".format(robust_auc, robust_p, robust_r, robust_f1))

    # os.makedirs(save_dir, exist_ok=True)
    # with open(os.path.join(save_dir, "results.txt"), 'w') as f:
    #     f.write("The carb scores AUC: {}, P: {}, R: {}, F1: {}\n".format(carb_auc, carb_p, carb_r, carb_f1))
    #     f.write("The robust scores AUC: {}, P: {}, R: {}, F1: {}\n".format(robust_auc, robust_p, robust_r, robust_f1))
    return carb_scores



# python scorer.py -pred result.json -gold gold.json -s ./
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--pred_file", "-pred", type=str, help="The predicted json file with the unified ROBUST format.")
    parser.add_argument("--gold_file", "-gold", type=str, help="The gold ROBUST json file.")  
    parser.add_argument("--save_dir", "-s", type=str, help="The directory to save results.")
    args = parser.parse_args()
    
    scores = main(args.gold_file, args.pred_file, args.save_dir)
    np.save("score4932.npy", scores)